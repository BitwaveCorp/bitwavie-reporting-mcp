...
Only return valid JSON. Do not include any explanations or additional text outside the JSON structure.`
  ...
  public async translateQuery(
    ...
    try {
      ...
      // TEMPORARILY DISABLED: Report detection for testing analyze mode
      ...
      //       description: `Detected request for predefined report: ${reportDetection?.reportType || 'Unknown'}`
      //     },
      //     {
      ...
      // Create a dummy reportDetection object to avoid TypeScript errors
      const reportDetection = {
        ...
        suggestedReports: [] as { name: string; confidence: number }[]
      };
